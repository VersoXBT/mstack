...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"m-social","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.mstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/mstack/bin/mstack-timeline-log '{"skill":"m-social","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
find . -name "*social*" -o -name "*.md" -path "*/posts/*" 2>/dev/null | head -5
```

Parse the user's request. Determine:
- What to post about (topic, announcement, content piece, feature, insight)
- Which platforms (all by default, or specific ones)

If the topic is unclear, use AskUserQuestion:
> "What's this post about?
> A) Promoting a piece of content (blog, guide, video)
> B) Announcing a product update or launch
> C) Sharing a take or insight from your space
> D) Something else — describe it"

Then ask:
> "Which platforms?
> A) All platforms (Twitter/X, LinkedIn, Reddit, Instagram)
...
If the user is promoting existing content, read it:
```bash
cat {content file path} 2>/dev/null | head -50
```
...
**Option B — Insight/take:**
```
{tweet option B}
```
{char count}/280

**Option C — Direct announcement (if applicable):**
```
{tweet option C}
```
{char count}/280

**Thread hook (if this deserves a thread):**
```
...
- Secondary: r/{subreddit 2} — why this community
...
Present all posts. Use AskUserQuestion:
> "Here are the social posts. What would you like to change?
> A) Adjust a specific platform's post
> B) Make the tone more {casual/bold/formal}
...
Suggest next steps:
- "Run `/m-threads` to write a full Twitter thread on this topic"
- "Run `/m-calendar` to schedule these posts in a content calendar"
- "Run `/m-repurpose` if you have more content to convert"
