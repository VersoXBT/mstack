...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"m-competitive","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.mstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/mstack/bin/mstack-timeline-log '{"skill":"m-competitive","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
```bash
# Check if brand.yaml needs updating
