#!/usr/bin/env bun
/**
 * analytics — CLI for viewing mstack skill usage statistics.
 *
 * Reads ~/.mstack/analytics/skill-usage.jsonl and displays:
...
const ANALYTICS_FILE = path.join(os.homedir(), '.mstack', 'analytics', 'skill-usage.jsonl');
...
export function formatReport(events: AnalyticsEvent[], period: string = 'all'): string {
  ...
  const lines: string[] = [];
  lines.push('mstack skill usage analytics');
