 ...
 * AskUserQuestion and persists the preference via mstack-config.
 */
...
\`\`\`bash
_CROSS_PROJ=$(${ctx.paths.binDir}/mstack-config get cross_project_learnings 2>/dev/null || echo "unset")
echo "CROSS_PROJECT: $_CROSS_PROJ"
if [ "$_CROSS_PROJ" = "true" ]; then
  ${ctx.paths.binDir}/mstack-learnings-search --limit 10 --cross-project 2>/dev/null || true
else
  ${ctx.paths.binDir}/mstack-learnings-search --limit 10 2>/dev/null || true
fi
\`\`\`
...
If A: run \`${ctx.paths.binDir}/mstack-config set cross_project_learnings true\`
If B: run \`${ctx.paths.binDir}/mstack-config set cross_project_learnings false\`
