...
description: |
  ...
  schema markup, and URL structure. Outputs an actionable checklist with fixes.
allowed-tools:
  - Bash
  - Read
  - Write
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"m-seo","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.mstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
| Keyword near start | Pass/Fail | {position} |
| Brand name format | Pass/Fail | {format check} |
| Click-worthy | Pass/Fail | {assessment} |

If issues found, provide a fixed version:
- Current: `{current title}`
- Recommended: `{improved title}` ({char count} chars)
...
- [ ] No broken internal links (check with browse if available)
- [ ] Most important pages linked to from this content
...
1. Link to {related page} with anchor text "{suggested anchor}"
2. Link to {related page} with anchor text "{suggested anchor}"
...
For each image missing alt text, suggest:
- `alt="{descriptive text with keyword where natural}"`

## Step 8: Schema Markup Check

Look for existing schema markup:
```bash
grep -i "schema.org\|application/ld+json\|itemtype" {file} 2>/dev/null | head -10
```

Based on content type, recommend schema:
- Blog post → `Article` schema
- Product page → `Product` schema
- FAQ section → `FAQPage` schema
- How-to content → `HowTo` schema

Provide a basic schema snippet if missing.

## Step 9: URL Structure (if URL provided)
...
Suggest improved URL if needed.

## Step 10: Compile Checklist
