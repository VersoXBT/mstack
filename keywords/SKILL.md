...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"m-keywords","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.mstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/mstack/bin/mstack-timeline-log '{"skill":"m-keywords","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
Suggest next steps:
- "Run `/m-brief` to create a content brief for any of these keywords"
- "Run `/m-seo` to optimize existing content for target keywords"
- "Run `/m-strategy` to incorporate these keywords into the content calendar"
